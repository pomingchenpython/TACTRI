...
def login1(request):  #登入
	...
	if request.method == 'POST':  #如果是以POST方式才處理
		...
		if user1 is not None:  #驗證通過
			if user1.is_active:  #帳號有效
				auth.login(request, user1)  #登入
				return redirect('/adminmain1/')  #開啟管理頁面
			else:  #帳號無效
				message = '帳號尚未啟用！'
		else:  #驗證未通過
			message = '登入失敗！'
...
def logout1(request):  #登出
	auth.logout(request)
	return redirect('/index/')
def adminmain1(request):  #管理首頁
	map_list = maplist1.objects.all().order_by('-id')  #遞減排序取得所有景點
	paginator = Paginator(map_list, 6)  #每頁資料筆數
	page = request.GET.get('page')  #取得目前頁數
	try:
		maps = paginator.page(page)
		...
		maps = paginator.page(paginator.num_pages)
	return render(request, "adminmain1.html", locals())

def adminadd1(request):  #新增景點
	if ('mapName' in request.POST):  #按確定新增鈕
		name = request.POST['mapName']
		desc = request.POST['mapDesc']   
		lat = request.POST['mapLat']
		...
		tel = request.POST['mapTel']
		addr = request.POST['mapAddr']
		rec=maplist1(mapName=name,mapDesc=desc,mapLat=lat,mapLng=lng,mapTel=tel,mapAddr=addr)  #建立資料記錄
		rec.save()  #寫入資料表
		return redirect('/adminmain1/')
	return render(request, "adminadd1.html", locals())
	
def adminedit1(request,editid=None):  #修改景點資料
	if editid != None:  #管理首頁按編輯鈕
		rec=maplist1.objects.get(id=editid)  #取得景點資料
		return render(request, "adminedit1.html", locals())
	else:  #按確定更新鈕
		editid1 =request.POST['editid']  #取得景點編號
		rec=maplist1.objects.get(id=editid1)  #取得景點資料
		rec.mapName=request.POST['mapName']  #取得景點欄位值
		rec.mapDesc=request.POST['mapDesc'] 
		rec.mapLat=request.POST['mapLat'] 
		rec.mapLng=request.POST['mapLng'] 
